...
if (process.env.uws !== "true" && process.env.cluster !== "true") {
    console.log("NO uWebSocket; NO cluster");
    var wsapp = express();
...
} else if (process.env.uws !== "true" && process.env.cluster === "true") {
    const numCPUs = require("os").cpus().length;
    const cluster = require("cluster");
    const http = require("http");
    const {setupMaster, setupWorker} = require("@socket.io/sticky");
    const {createAdapter, setupPrimary} = require("@socket.io/cluster-adapter");
    console.log(`NO uWebSocket; cluster mode using ${numCPUs} cpu-cores`);
